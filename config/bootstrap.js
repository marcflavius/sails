...
module.exports.bootstrap = async function () {
  ...
  if (sails.config.models.migrate !== 'drop' && sails.config.environment !== 'test') {
    ...
    if (process.env.NODE_ENV === 'production' || sails.config.models.migrate === 'safe') {
      sails.log('Since we are running with migrate: \'safe\' and/or NODE_ENV=production (in the "' +
    ...
    sails.log('Running v' + HARD_CODED_DATA_VERSION + ' bootstrap script...  (' + (lastRunBootstrapInfo
      ? 'before this, the last time the bootstrap ran on this computer was for v' +
              lastRunBootstrapInfo.lastRunVersion + ' @ ' + (new Date(lastRunBootstrapInfo.lastRunAt))
  ...
  const admin1 = await User.findOne({emailAddress: 'admin@example.com'});
  const admin2 = await User.findOne({emailAddress: 'admin2@example.com'});
    ...
    force: true,
             ...
             .tolerate((err) => {
               sails.log.warn(
                 ...
                 sails.config.appPath + '`.  Full error details: ' + err.stack +
